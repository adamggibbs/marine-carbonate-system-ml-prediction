# JOB: Run CANYONB pH estimates on testing dataset 
# AUTHOR: Adam Gibbs, MBARI Intern
# DATE: July 19, 2021
################################################################################
# start of script
...
# store directory where necessary files are
dir <- '~/Documents/csumb-mbari-2021/CANYONB_estimates/'

# initialize CANYONB function as presented by Bittig et al, 2018
# copy code from GitHub: 
# https://github.com/HCBScienceProducts/CANYON-B/blob/master/CANYONB.R
CANYONB <- function(date,lat,lon,pres,temp,psal,doxy,param,
                    epres,etemp,epsal,edoxy){
  ...
  inputsdir=dir # relative or absolute path to CANYON-B wgts files
...
# create wrapper function to apply CANYONB to inputs
# takes a vector of inputs, applies inputs to CANYONB, returns pH estimate
CANYONB_apply <- function(x){
  # get inputs from vector and properly casts them
  # gets estimate vector as output from CANYONB function
  estimate <- CANYONB(date=x[1], 
                      lat=as.double(x[2]), 
                      lon=as.double(x[3]), 
                      pres=as.double(x[4]), 
                      temp=as.double(x[5]), 
...
inputs <- read.csv(paste(dir, 'canyonb_testing_tpso_input.csv', sep=""))

estimates <- apply(inputs, MARGIN=1, FUN=CANYONB_apply)

# convert estimates list to numeric vector
#estimates <- list(as.numeric(unlist(estimates)))

# peek at final 6 pH estimates to get an idea if everything worked
tail(estimates)

# save outputs in a csv file
fwrite(estimates, paste(dir, 'canyonb_pH_outputs.csv', sep=""),
